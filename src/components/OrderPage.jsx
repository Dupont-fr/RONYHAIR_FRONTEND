...
*Voir l'image:* ${product.url}`
      ...
      const whatsappUrl = `https://api.whatsapp.com/send?phone=${WHATSAPP_NUMBERS.MAIN}&text=${encodeURIComponent(messageTexte)}`
      window.open(whatsappUrl, '_blank')

      await analyticsService.trackCommande(product._id, product.nom, product.categorie, 'Catégorie')
  ...
  return (
    <div className='order-page-wrapper'>
            ...
            <div className='product-info'>
              <div className='info-row'>
                ...
                <span className='info-value price'>
                  {product.prix > 0 ? `${product.prix.toLocaleString()} ${product.devise || 'FCFA'}` : 'Sur demande'}
                </span>
              </div>
          ...
          <div className='form-card'>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className='form-row-2col'>
                ...
                <div className='form-inputs-row'>
                  <input type='tel' name='telephone' value={formData.telephone} onChange={handleChange} required placeholder='+237 6XX XXX XXX' />
                  <input type='text' name='ville' value={formData.ville} onChange={handleChange} required placeholder='Douala, Yaoundé...' />
                </div>
              </div>

              <div className='form-row-full'>
                <label>Adresse *</label>
                <input type='text' name='adresse' value={formData.adresse} onChange={handleChange} required placeholder='Votre adresse complète' />
              </div>

              <button type='submit' className='btn-submit' style={{ background: '#0F1B3D' }} disabled={submitting}>
      ...
      <footer className='order-page-footer'>
        <p>&copy; {new Date().getFullYear()} RONY HAIR 237 - Institut de Beauté & Bien-être</p>
      </footer>
    </div>
  )
}
